...
const Book = () => {
    ...
    const handlePayment = async () => {
        ...
        let newCustomer: Customer=null;

        if (user) {
            try {
                const parsedUser = JSON.parse(user); // Parse user data if it's stored as a string
                if (parsedUser.id) {
                    newCustomer = await dispatch(getCustomer('67e58cce7b5b240f043f4329')).unwrap();
                } else {
                    console.error("Id not found in user data.");
                }
            } catch (error) {
                console.error("Error parsing user data or dispatching action:", error);
            }
        } else {
                            ...
                            {step === 'confirmation' && (
                                <div className="text-center space-y-4">
                                    ...
                                    <p className="text-muted-foreground">
                                        Thank you for your booking. We've sent a confirmation email with all the
                                        details.
                                    </p>
